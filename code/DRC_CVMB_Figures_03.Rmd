...
Figure scripts for the paper, "The vaginal microbiome of pregnant people living with HIV on antiretroviral therapy in the Democratic Republic of Congo: A Pilot Study and Global Meta-Analysis"
...
run_PCA_biplot(ps = ps_congo_filt_spp_rclr,
        batch = "CST", 
        scale = FALSE, 
        title = "",
        color_palette = CST_col,
        n_loadings = 5) + labs(caption = paste("ANOSIM R = 0.2058,  p = 0.001"))


# ps_congo_filt_spp_rclr %>%
#   microViz::ord_calc(method = "PCA", scale_cc = FALSE) %>%
#   microViz::ord_plot(color = "CST", plot_taxa = 1:5, size = 5) +
...
all_methods <- read_csv("tables/DAA/all_methods.csv")
...
#from reviwer_order by abundnace
species_abundance_diff <- ps_melted_clr %>%
  filter(id %in% daa_spp) %>%
  group_by(id, health_status) %>%
  summarise(mean_abundance = mean(abundance, na.rm = TRUE), .groups = 'drop') %>%
  pivot_wider(names_from = health_status, values_from = mean_abundance) %>%
  # Calculate difference: PWoH abundance - PLWH-ART abundance
  mutate(abundance_diff = Asymptomatic - HIV) %>%  # Assuming "Asymptomatic" = PWoH and "HIV" = PLWH-ART
  arrange(desc(abundance_diff)) %>%  # Sort by highest PWoH abundance to highest PLWH-ART abundance
  mutate(id_formatted = gsub("f__|_g__|_s__", " ", id),
         id_formatted = gsub("^ ", "", id_formatted),
         id_formatted = gsub("([^ ]+)( )", "\\1\n", id_formatted))
...
ps_melted_clr %>%
  ...
  mutate(id_formatted = gsub("f__|_g__|_s__", " ", id),
         id_formatted = gsub("^ ", "", id_formatted),
         id_formatted = gsub("([^ ]+)( )", "\\1\n", id_formatted)) %>%
  # Convert to factor with levels ordered by abundance difference (PWoH > PLWH-ART)
  mutate(id_formatted = factor(id_formatted, 
                               levels = species_abundance_diff$id_formatted)) %>%
  ggplot(aes(x = id_formatted, y = abundance, 
             fill = health_status)) +
  geom_boxplot(coef = 100, width = 0.5) +
  scale_fill_manual(values = c("HIV" = "#DC3220", "Asymptomatic" = "#005AB5"),
                    labels = c("HIV" = "PLWH-ART", "Asymptomatic" = "PWoH")) + 
...
## Figure 6
```{r, echo=TRUE, fig.height = 28, fig.width = 20}
# load files
ps_meta_picrust = readRDS("data/ps_meta_picrust.RDS")
# health Status
all_methods_meta <- read_csv("tables/DAA/all_methods_picrust_meta.csv")
...
###REVIWER SUGGESTION- add two sided pvalues
 results_table_asv <- data.frame(
   Cytokine = character(),
   OTU = character(),
   Taxon = character(),
   Rho = numeric(),
   PValue = numeric(),
   TwoSided_PValue = numeric(),
 ...
 for (i in 1:nrow(corr_results_df)) {
   ...
   cytokine_name <- corr_results_df$Cytokine[i]
   taxon_name <- corr_results_df$Taxon[i]
   otu_name <- corr_results_df$OTU[i]
   rho <- corr_results_df$Rho[i]
   p_value <- corr_results_df$PValue[i]
   
   # Calculate two-sided p-value from correlation test
   x <- otu_cyt[[otu_name]]
   y <- meta_cyt_log[[cytokine_name]]
   cor_test <- cor.test(x, y, method = "spearman")  # or "pearson" depending on your method
   two_sided_p <- cor_test$p.value
   
   # Add to results table
   results_table_asv <- rbind(results_table_asv, 
                          data.frame(Cytokine = cytokine_name,
                                     Rho = rho,
                                     OTU = otu_name,
                                     Taxon = taxon_name,
                                     PValue = p_value,
                                     TwoSided_PValue = two_sided_p,
                                     Adjusted_PValue = NA,  # Will fill this after the loop
                                     stringsAsFactors = FALSE))
 }
 
 # Apply Benjamini-Hochberg correction to the two-sided p-values
 results_table_asv$Adjusted_PValue <- p.adjust(results_table_asv$PValue, method = "BH")
# write.csv(results_table_asv, "correlation_results_taxa_cyt.csv", row.names = FALSE)

 # Define the top 3 OTUs
top3 <- c("ASV1", "ASV2", "ASV4")

# Filter results for the top 10 OTUs and significant p-values (PValue <= 0.05)
corr_results_df_top3 <- results_table_asv %>%
  filter(OTU %in% top3) %>%
  filter(TwoSided_PValue <= 0.05)
...
# write.csv(corr_results_alpha, "stats/congo/shannon_cytokine_correlation_results.csv", row.names = FALSE


#add two sided pvalues
...
# Apply Benjamini-Hochberg correction to the two-sided p-values
results_table$Adjusted_PValue <- p.adjust(results_table$PValue, method = "BH")

# Print the table
#print(results_table)

# Optional: Write to CSV file
 #write.csv(results_table, "correlation_results_shannon_cyt.csv", row.names = FALSE)


```


## A-B.Correlation of immune factors to shannon diversity

```{r, echo=TRUE}
# Loop through each row in the results_df_alpha data frame
for (i in 1:nrow(results_table)) {
  # Extract the cytokine name, rho, and p-value
  cytokine_name <- results_table$Cytokine[i]
  rho <- results_table$Rho[i]
  p_value <- results_table$TwoSided_PValue[i]
...
for (i in 1:nrow(corr_results_df_top3)) {
  ...
  p_value <- corr_results_df_top3$TwoSided_PValue[i]
...
# Save the results to a CSV file
# write.csv(corr_results_df_cst, file = "stats/congo/cytokine_cst_correlations2.csv", row.names = FALSE)
...
## Fig S3: Heatmap of correlations between all taxa and immune mediators in the CQI-PMTCT cohort

```{r, echo=TRUE, fig.width=11, fig.height=10}
#corr_df <- read.csv("tables/linear_regression_results_log.csv", header = T, sep = ";")
corr_df <- results_table_asv
...
figs2<- corr_df %>% 
   ...
   geom_point(
     data = function(x) filter(x, TwoSided_PValue < 0.05),
     shape = "asterisk", size = 2
   ) +
   geom_point(
     data = function(x) filter(x, Adjusted_PValue < 0.05),
     shape = "circle", size = 3
   ) +
 ...
 ggsave("figS2_revision1.pdf", figs2, width = 10, height = 10)
```

## Fig S2: Covariates explaining microbial variation in the CVMB of the integrated datasets
...
```
##Suggested by reviewer: All Cytokine and CST plots
```{r, echo=TRUE, warnings = FALSE, fig.width=20, fig.height=20}

library(patchwork)

plots <- lapply(names(meta_cyt_log2)[3:46], function(col) {
  
  p <- ggplot(meta_cyt_log2 %>% filter(!is.na(CST)), aes_string(x = "CST", y = col, fill = "CST")) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(color = "black", fill = "white", alpha = 0.7, height = 0) +
    labs(y = paste(sub("^enrol_", "", col)), x = " ", 
         caption = paste("p =", round(wilcox.test(meta_cyt_log2[[col]] ~ meta_cyt_log2$CST)$p.value, 4))) +
...
## Suggested by reviewer: All shannon and cytokine linear plots
```{r, echo=TRUE, fig.width=20, fig.height=20}

n_plots <- nrow(corr_results_alpha)
...
for (i in 1:n_plots) {
  ...
  # Remove "enrol_" prefix from cytokine name
  clean_cytokine_name <- gsub("^enrol_", "", cytokine_name)
  ...
  # Generate the scatter plot
plot(x, y, 
     main = paste("Shannon Entropy vs", clean_cytokine_name,
                  "\nr =", round(rho, 3), "| p =", round(p_value, 3)),
     xlab = "Shannon Entropy",
     ylab = clean_cytokine_name)
abline(lm(y ~ x), col = "blue")

# Add 95% CI
model <- lm(y ~ x)
newx <- seq(min(x), max(x), length.out = 100)
pred <- predict(model, newdata = data.frame(x = newx), interval = "confidence")
polygon(c(newx, rev(newx)), c(pred[, "lwr"], rev(pred[, "upr"])), 
...
## Suggested by reviewer: linear plots of all taxon and cytokines
```{r, echo=TRUE, fig.width=5, fig.height=5}
n_plots <- nrow(results_table_asv)
n_cols <- ceiling(sqrt(n_plots))
n_rows <- ceiling(n_plots / n_cols)
...
for (i in 1:n_plots) {
  # Extract the cytokine name, rho, and p-value
  cytokine_name <- results_table_asv$Cytokine[i]
  otu_name <- results_table_asv$OTU[i]
  rho <- results_table_asv$Rho[i]
  p_value <- results_table_asv$TwoSided_PValue[i]
...
#    plot(x, y, 
#     main = paste(otu_name, "vs", clean_cytokine_name,
#                  "\nr =", round(rho, 3), "| p =", round(p_value, 3)),
#     xlab = otu_name,
#     ylab = clean_cytokine_name)
#abline(lm(y ~ x), col = "blue")
...
#abline(model, col = "grey")
  ...
  plot(ggplot(data = data.frame(x = x, y = y), aes(x = x, y = y)) +
      ...
      geom_smooth(method = "lm", color = "blue") +
      labs(
        title = paste(otu_name, "vs", clean_cytokine_name),
        subtitle = paste("r =", round(rho, 3), "| p =", round(p_value, 3)),
        x = otu_name,
        y = clean_cytokine_name
      ) +
      theme_minimal())
  }
}
...
## Covariance and correlation plot of all immune factors
```{r, echo=TRUE, fig.width=15, fig.height=10}
set.seed(2025)
#compute corr and covariance and turn to tables
cyt_cov_mat <- cov(meta_cyt_log, use = "everything", method = "spearman")

cyt_cor_mat <- cor(meta_cyt_log, use = "complete.obs", method = "spearman")


#calculate significcance
cor_test_result <- corr.test(meta_cyt_log, use = "complete.obs", method = "spearman")
cor_p_matrix <- cor_test_result$p 
#convert to sign stars matrix
sig_stars <- matrix("", nrow = nrow(cor_p_matrix), ncol = ncol(cor_p_matrix))
sig_stars[cor_p_matrix < 0.001] <- "***"
sig_stars[cor_p_matrix >= 0.001 & cor_p_matrix < 0.01] <- "**"
sig_stars[sig_stars == "" & cor_p_matrix < 0.05] <- "*"
sig_stars[sig_stars == "" & cor_p_matrix >= 0.05] <- ""


cov_table <- as.data.frame(cyt_cov_mat) %>% 
  rownames_to_column("Cytokine") %>% 
  mutate_if(is.numeric, round, 3)

#write.csv(cov_table, "Covariance_cytokines.csv", quote = F)

cor_table_long <- as.data.frame(cyt_cor_mat) %>%
  rownames_to_column("Cytokine1") %>%
  pivot_longer(cols = -Cytokine1, names_to = "Cytokine2", values_to = "Correlation") %>%

  mutate(p_value = map2_dbl(Cytokine1, Cytokine2, 
                           ~cor_p_matrix[.x, .y])) %>%
  ...
  mutate(pair = map2_chr(Cytokine1, Cytokine2, ~paste(sort(c(.x, .y)), collapse = "-"))) %>%
  ...
  mutate(
    ...
    p_value_formatted = ifelse(p_value < 0.0001, "<0.0001", 
                              sprintf("%.4f", p_value))
  ) %>%
  arrange(desc(abs(Correlation)))

#write.csv(cor_table_long, "Correlations_cytokines.csv", quote = F)

#plot covariance matrix
pheatmap(cyt_cov_mat,
         main = "Covariance Matrix of Immune factors",
         color = colorRampPalette(c("green4", "white", "purple4"))(100),
...
#plot correlatio matrix with signficance
corrplot(cyt_cor_mat,
         method = "circle",
         type = "upper",
         tl.col = "black", 
         tl.srt = 45,
         main = "Correlation Matrix of Immune factors",
         mar = c(0, 0, 2, 0))
